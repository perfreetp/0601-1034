(function () {
  ...
  function loadState() {
    return new Promise(resolve => {
      chrome.storage.local.get('metavenue_state', data => {
        if (data.metavenue_state) {
          state = { ...state, ...data.metavenue_state };
        }
        resolve();
      });
    });
  ...
  function collectCurrentState() {
    ...
    state.space.posterUrl = $('#poster-url').value;
    state.space.bgmUrl = $('#bgm-url').value;
    state.space.bgmVolume = parseInt($('#bgm-volume').value);
    ...
    state.seats.vipLabel = $('#vip-label').value;

    state.agenda.liveUrl = $('#live-url').value;
    state.agenda.liveBound = !$('#live-status').classList.contains('hidden');
    state.agenda.countdownStart = $('#countdown-start').value;
    state.agenda.countdownTitle = $('#countdown-title').value;
    state.agenda.countdownVisible = $('#countdown-visible').checked;
    state.agenda.items = $$('.agenda-item').map(item => ({
      time: $(item, '.agenda-time input').value,
      name: $(item, '.agenda-name input').value
    }));
    ...
    state.interact.danmakuOpacity = parseInt($('#danmaku-opacity').value);
    ...
    state.booth.cards = $$('.booth-card-item').map(item => ({
      title: $(item, '.booth-title').value,
      url: $(item, '.booth-url').value,
      color: $(item, '.booth-card-color').style.background
    }));
    state.booth.radius = parseInt($('#booth-radius').value);
    state.booth.gap = parseInt($('#booth-gap').value);

    state.guide.hotspots = $$('.hotspot-item').map(item => ({
      name: $(item, '.hotspot-name').value,
      url: $(item, '.hotspot-url').value
    }));
    state.guide.narrations = $$('.narration-item').map(item => ({
      time: $(item, '.narration-time input').value,
      text: $(item, '.form-textarea').value
    }));
    state.guide.ttsVoice = $('#tts-voice').value;
    state.guide.ttsRate = parseInt($('#tts-rate').value);
  ...
  function initAgendaPanel() {
    ...
    $('#btn-add-agenda').addEventListener('click', () => {
      ...
      item.innerHTML = `
        <div class="agenda-time"><input type="text" class="form-input agenda-input" placeholder="时间"></div>
        <div class="agenda-name"><input type="text" class="form-input agenda-input" placeholder="环节名称"></div>
    ...
    $$('.agenda-item').forEach(bindRemoveBtn);
  ...
  function initBoothPanel() {
    ...
    $('#btn-add-booth').addEventListener('click', () => {
      ...
      item.innerHTML = `
        ...
        <input type="text" class="form-input booth-title" placeholder="展台名称">
        <input type="text" class="form-input booth-url" placeholder="链接 URL">
        <button class="btn-icon btn-remove" title="删除">✕</button>
      `;
      list.appendChild(item);
      bindRemoveBtn(item);
    ...
    $$('.booth-card-item').forEach(bindRemoveBtn);
  }
  ...
  function initGuidePanel() {
    $('#btn-add-hotspot').addEventListener('click', () => {
      ...
      item.innerHTML = `
        ...
        <input type="text" class="form-input hotspot-name" placeholder="热点名称">
        <input type="text" class="form-input hotspot-url" placeholder="跳转 URL">
        <button class="btn-icon btn-remove" title="删除">✕</button>
      `;
      list.appendChild(item);
      bindRemoveBtn(item);
    ...
    $$('.hotspot-item').forEach(bindRemoveBtn);
    ...
    $('#btn-add-narration').addEventListener('click', () => {
      ...
      item.innerHTML = `
        <div class="narration-header">
          <span class="narration-time"><input type="text" class="form-input sm" placeholder="时间"></span>
          <button class="btn-icon btn-play-narration" title="播放">▶</button>
        </div>
        <textarea class="form-textarea" placeholder="口播内容"></textarea>
      `;
      list.appendChild(item);
      bindNarrationPlay(item);
      collectCurrentState();
    });

    $$('.narration-item').forEach(bindNarrationPlay);
  ...
  function initReviewPanel() {
    $('#btn-export-csv').addEventListener('click', () => {
      ...
      downloadFile(csv, 'metavenue-review.csv', 'text/csv');
    });

    $('#btn-export-json').addEventListener('click', () => {
      collectCurrentState();
      const json = JSON.stringify(state.review, null, 2);
  ...
  function refreshReviewPanel() {
    chrome.storage.local.get('metavenue_analytics', data => {
      const analytics = data.metavenue_analytics || { participants: 0, avgDuration: 0, interactions: 0, questions: 0, logs: [], questionList: [] };
  ...
  function generateCSV() {
    const analytics = state.review;
    let csv = '类型,时间,内容\n';
    if (analytics.logs) {
      analytics.logs.forEach(l => {
        csv += `${l.typeLabel},${l.time},${l.content}\n`;
      });
    }
    return csv;
  }
  ...
  function bindNarrationPlay(item) {
    const btn = $(item, '.btn-play-narration');
    if (btn) {
      btn.addEventListener('click', () => {
        const text = $(item, '.form-textarea').value.trim();
        if (text) {
  ...
  function sendToContent(action, data) {
    chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
      if (tabs[0] && tabs[0].id) {
        chrome.tabs.sendMessage(tabs[0].id, { source: 'metavenue-popup', action, data });
      }
    });
  }
  ...
  function restoreUI() {
    ...
    if (state.seats.count) $('#seat-count').value = state.seats.count;
    if (state.seats.perRow) $('#seats-per-row').value = state.seats.perRow;
    $('#vip-enabled').checked = state.seats.vipEnabled;
    if (state.seats.vipCount) $('#vip-count').value = state.seats.vipCount;
    ...
    if (state.agenda.notifyAdvance) $('#notify-advance').value = state.agenda.notifyAdvance;
    ...
    if (state.interact.danmakuOpacity) {
      ...
      if (msg.source === 'metavenue-content') {
        if (msg.action === 'handraise') {
          ...
          if (queueItems) {
            ...
            item.querySelector('.btn-icon').addEventListener('click', () => item.remove());
          }
